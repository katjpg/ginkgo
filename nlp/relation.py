    ...
    def _classify(self, pair: dict) -> dict:
        ...
        try:
            response = self.client.models.generate_content(
                model=self.config.model_id, contents=prompt, config=config
            )
...
def extract_with_aggregation(
    entities: list[dict], doc: Doc, extractor: RelationExtractor
) -> list[dict]:
    ...
    for entity_pair in aggregated:
        ...
        for mention in entity_pair.mentions[
            :3
        ]:  # limit to 3 mentions for context length
